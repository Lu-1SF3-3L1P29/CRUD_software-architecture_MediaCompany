...
def listaSub():
    ...
    querySQL = "SELECT * FROM subs ORDER BY id DESC"
...
def updateSub(id=''):
        ...
        cursor.execute("SELECT * FROM subs WHERE id = %s LIMIT 1", [id])
        resultQueryData = cursor.fetchone() #Devolviendo solo 1 registro
        return resultQueryData
    
    
    
def registrarSub(nombre='', edad='', nuevoNombreFile=''):       
        conexion_MySQLdb = connectionBD()
        cursor           = conexion_MySQLdb.cursor(dictionary=True)
            
        sql         = ("INSERT INTO subs(nombre, edad, foto) VALUES (%s, %s, %s)")
        valores     = (nombre, edad, nuevoNombreFile)
...
def detallesSub(idSub):
        conexion_MySQLdb = connectionBD()
        cursor = conexion_MySQLdb.cursor(dictionary=True)
        
        cursor.execute("SELECT * FROM subs WHERE id ='%s'" % (idSub,))
...
def  recibeActualizarSub(nombre, edad, nuevoNombreFile, idSub):
        conexion_MySQLdb = connectionBD()
        cur = conexion_MySQLdb.cursor(dictionary=True)
        cur.execute("""
            UPDATE subs
            SET 
                ...
                foto    = %s
            WHERE id=%s
            """, (nombre, edad, nuevoNombreFile,  idSub))
        conexion_MySQLdb.commit()
